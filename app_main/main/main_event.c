...
void main_state_timeout_cb(uint32_t *arg)
{
   ...
   // {
   //    //switch to HTTP
   //    main_mon->timeout_cb = main_http_timeout_cb;
   //    os_timer_arm(&main_mon->timer, 5000, 0);
   // } else {
   //    // save transport
   // }
}
 ...
 * on 500 ms interval. Once a timestamp is obtained, arms main_state_timeout_cb
 * to begin pushing state info to server.
 */
static void main_ts_timeout_cb(uint32_t *arg)
{
   os_timer_disarm(&main_mon->timer);
   ...
   {
      os_timer_arm(&main_mon->timer, 1053, false);
   }
}
 ...
 */
static void main_reconnect_cb(uint32_t *arg)
{
   os_timer_disarm(&main_mon->timer);
   MAIN_EVENT_DBG("main_reconnect_cb:\n");
   NODE_DBG("main_reconnect_cb:\n");

   /* Make sure MQTT isn't connected still */
   if (main_api_get_conn_state() != MAIN_CONN_MQTT)
   {
      MAIN_EVENT_DBG("\tno mqtt\n");
      /* Try again */
      wifi_station_connect();
      os_timer_arm(&main_mon->timer, RECONNECT_TIMOUT, 0);
   }

   /* MQTT connected, re-arm sntp timer */
   else
   {
      MAIN_EVENT_DBG("\tmqtt connected!\n");
...
void arm_state_timer(int dt)
{
   ...
   main_mon->timeout_cb = main_state_timeout_cb;

   /* Arm timer w device specifc timeout */
   os_timer_arm(&main_mon->timer, 15021U, 0);
}
...
void arm_sntp_timer()
{
   ...
   main_mon->timeout_cb = main_ts_timeout_cb;

   /* Arm timer for a bit over 3 seconds. SNTP timeout is 3 sec. */
   os_timer_arm(&main_mon->timer, SNTP_TIMEOUT, 0);
}

/* Arm reconnect timer */
void arm_reconnect_timer(uint32_t delay)
{
   // MAIN_EVENT_DBG("arm_reconnect_timer\n");
   NODE_DBG("arm_reconnect_timer:\n");
   /* Disarm */
   os_timer_disarm(&main_mon->timer);

   /* Swap in the ts callback pointer */
   main_mon->timeout_cb = main_reconnect_cb;

   /* Arm timer */
   os_timer_arm(&main_mon->timer, delay, 0);
}
...
static void Event_Mon_Task(os_event_t *e)
{
   ...
   MAIN_IO_MSG_T * msg = (MAIN_IO_MSG_T *)e->par;
   if (msg)
   {
      type = msg->type;
      msgLen = msg->len;
...
static void nslookup_cb(const char *name, ip_addr_t *ip, void *arg)
{
   ...
   else
   {
      ...
      if (ipaddr_ntoa_r(ip, ipstr, sizeof(char)*17) != NULL)
      {
         ...
         main_mon->mqtt->mqtt_host_ip = ipstr;
         /* Init MQTT */
         mqtt_app_init(main_mon->mqtt);
         mqtt_setconn(1);
   ...
   switch (evt->event)
   {
      ...
      case EVENT_STAMODE_DISCONNECTED:
      {
         ...
         if (wifiState != wifiIsDisconnected)
         {
            ...
            main_mon->conn_state = MAIN_CONN_DISCONNECTED;
...
void main_event_init(MAIN_MON_T * clientPtr)
{
   ...
   main_mon->timeout_cb = main_ts_timeout_cb;
	// os_timer_arm(&main_mon->timer, 2000, 0);
