...
static SETUP_MQTT_T * setup_mqtt = NULL;
...
static void mqttConnectedTestCb(uint32_t *args)
{
   ...
   if (MQTT_Subscribe(client, setup_mqtt->test_sub, 1))
   {
      setup_api_update_conn_state(SETUP_CONN_MQTT);
   }
}
...
static void mqttDisconnectedCb(uint32_t *args)
{
	...
	// setup_api_update_conn_state(SETUP_CONN_DISCONNECTED);
}
...
bool mqtt_api_pub_temp(char * msg, int len)
{
   ...
   return MQTT_Publish(&mqtt_client, setup_mqtt->test_pub, msg, len, 1, 0);
}
...
void mqtt_app_init(SETUP_MQTT_T * setup_mqtt_ptr)
{
	/* Attach MQTT Monitor struct to MQTT Client */
	setup_mqtt = setup_mqtt_ptr;
   setup_mqtt->mqtt_buff_avail_cb = mqtt_api_buff_len_avail;

	/* Init MQTT */
	MQTT_InitConnection(&mqtt_client, setup_mqtt_ptr->mqtt_host_ip, 1883, 0);
