...
void detect(uint8 *arg, uint16 len)
{
   ...
   if ( len == 12 ) return;
   else if (len >= 64) buf = arg + sizeof(struct RxControl);
   else return;


   if ( ( (buf[0]) & TYPE_SUBTYPE_MASK) != TYPE_SUBTYPE_QOS_DATA) return;

   if ( (buf[1] & DS_RETRY_MASK) != NO_RETRY ) return;

   // Fragment Number should = 0
   if ( buf[SEQ_ADDR] & 0xF != 0 ) return;
   ...
   // cur_base_seq is ref to flag[0] when finding the patern
   if (!matched)
   {
      int i;
      // for each source-dest adress pair in the map
      for (i = 0; i < ADDR_MAP_NUM; i++)
      {
         ...
         else if (memcmp(am[i]->addr, &buf[ADDR_MATCH_START], ADDR_MATCH_LENGTH) == 0)
         {
            ...
            // base_seq is not valid any more
            if (seq_delta == 0)
            {
               // lost the flag[0]
               if ( len - am[i]->base_len != am[i]->flag[0])
...
{
   mode = wifi_get_opmode();
   if ( (STATION_MODE != mode) && (mode != STATIONAP_MODE) ) return;
