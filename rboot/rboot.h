...
typedef struct {
   ...
   uint8 last_main_rom;   ///< Keeps track of which "main" rom we used last so we can switch back to that one
   uint8 last_setup_rom;  ///< Keeps track of which "setup" rom we were using last
...
static inline uint8 default_config(rboot_config *romconf, uint32 flashsize) {
   romconf->count = 4;
   romconf->roms[0] = SECTOR_SIZE * (BOOT_CONFIG_SECTOR + 1);
   romconf->roms[1] = (flashsize / 8) + (SECTOR_SIZE * (BOOT_CONFIG_SECTOR + 1));
   romconf->roms[2] = (flashsize / 4) + (SECTOR_SIZE * (BOOT_CONFIG_SECTOR + 1));
   romconf->roms[3] = (flashsize / 2) + (SECTOR_SIZE * (BOOT_CONFIG_SECTOR + 1));
}
#endif
